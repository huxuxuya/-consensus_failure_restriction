#!/usr/bin/env python3
"""Calculate case 2 compensation for mixed preserved lowered-weight nodes."""
...
DEFAULT_EPOCH = 247
DEFAULT_OUTPUT = "checks/case-2-preserved-node-lowered-weight/case_2_compensation.csv"
MAX_TABLE_N = 990
P0_MULTIPLIERS = {
    50: (1, 20),
    100: (1, 10),
...
def non_preserved_poc_weight(validation_weight: dict[str, Any]) -> int:
    total = 0
    for node in validation_weight.get("ml_nodes", []):
        slots = node.get("timeslot_allocation") or []
        if not (len(slots) > 1 and bool(slots[1])):
            total += to_int(node.get("poc_weight"))
    return total
...
def calculate(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for model_vw in model_group.get("validation_weights", []):
        preserved_weight = preserved_poc_weight(model_vw)
        non_preserved_weight = non_preserved_poc_weight(model_vw)
        if preserved_weight <= 0 or non_preserved_weight <= 0:
            continue
        ...
        expected_effective_weight = min(weight, confirmation_weight + preserved_weight)
        ...
        expected = (
            decimal_floor(
                Decimal(expected_effective_weight)
        ...
        rows.append(
            {
                ...
                "confirmation_to_weight_ratio": str(
                    Decimal(confirmation_weight) / Decimal(weight)
                ),
                "preserved_poc_weight": preserved_weight,
                "non_preserved_poc_weight": non_preserved_weight,
